...
// }
type Process struct {
	C       utils.Value // Значение загрузки процессора
	COMMAND utils.Value // Имя и аргументы команды
	MEM     utils.Value // Часть реальной памяти (%), используемая процессом
	PID     utils.Value // Идентификатор процесса
	PGID    utils.Value // Индентификатор группы
	PPID    utils.Value // Идентификатор родительского процесса
	PSR     utils.Value // Процесс, которому в данный момент назначен процесс
	RSS     utils.Value // Размер резидентного набора (количество страниц памяти)
	SID     utils.Value // Идентификатор сеанса
	START   utils.Value // Время создания процесса
	STAT    utils.Value /*
		Текущий статус процесса:
			R — выполняется
			D — ожидает записи на диск
	...
	*/
	SZ    utils.Value // Размер в физических страницах основного образа процесса
	TIME  utils.Value // Количество времени ЦП, затраченное на выполнение процесса
	TPGID utils.Value // Идентификатор группы процессов переднего плана на TTY (терминале), к которому подключен процесс, или -1 если процесс не связан с TTY
	TTY   utils.Value // Идентификатор управляяющего терминала
	UID   utils.Value // Эффективный идентификатор пользователя
	USER  utils.Value // Имя владельца процесса
	VSZ   utils.Value // Виртуальный размер процесса
}
